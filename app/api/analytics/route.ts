...
export async function GET(request: Request) {
  try {
    ...
    Object.keys(categoryAverages).forEach(category => {
      categoryAverages[category] /= feedbackDocs.length;
    });

    // Get improvement areas
    const improvementAreas = feedbackDocs.length > 0
      ? feedbackDocs[0].areasForImprovement
      : [];

    // Get strengths
    const strengths = feedbackDocs.length > 0
      ? feedbackDocs[0].strengths
      : [];

    // Calculate progress over time
    const progress = feedbackDocs.map(doc => ({
      date: doc.createdAt,
